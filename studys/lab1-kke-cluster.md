...
- 카카오클라우드 콘솔에 로그인하여 서비스 구성에 필요한 리소스(클러스터, 노드 풀)를 직접 생성한다.

#### 엔드포인트 설정
1. 퍼블릭 엔드포인트(사용)
   - 누구든지 로컬 노트북에서 쿠버네티스 클러스터의 API 서버로 바로 명령을 내릴 수 있음
2. 프라이빗 엔드포인트(실무 정석) = 오직 카카오클라우드 내부망(VPC) 안에서만 통신 가능
   - VPN 이나, 베스쳔 서버를 거쳐야만(베스쳔 ssh 연결 이후에나) 명령을 내릴 수 있다.(앞 네트워크 구축에서 베스쳔까지 만들기는 함..)
